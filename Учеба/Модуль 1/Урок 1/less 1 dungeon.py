...
def introduction():
    print(
        "Добро пожаловать в подземелье! Ты отважился войти в темные глубины, где могут произойти ужасные вещи."
    )
    time.sleep(1)
    print(
        "Твоя задача - дойти до конца подземелья, преодолевая препятствия на своем пути."
    )
    time.sleep(1)
    print(
        "Выбирай свои действия осторожно, и помни, что каждое решение влияет на твое приключение!\n"
    )
...
def dungeon_quest():
    ...
    if choice_1 == 1:
        ...
        choice_2 = make_choice(
            "2. Открыть дверь или вернуться обратно?",
            ["Открыть дверь", "Вернуться назад"],
        )
    ...
    choice_3 = make_choice(
        "1. Перейти мост осторожно.", ["Перейти", "Вернуться обратно"]
    )
    ...
    choice_5 = make_choice(
        "1. Использовать ключ.", ["Использовать ключ", "Попробовать взломать дверь"]
    )
